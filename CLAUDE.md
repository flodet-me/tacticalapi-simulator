...
- **Host and adapters are separate executables, one store.** `TacticalApi.Simulator.Host` is only the store + `Situation` gRPC service + map UI — it has zero data sources. Each source runs in its own `TacticalApi.Simulator.Adapter.*` process and pushes updates into whichever endpoint `Simulator:Ingest:Address` points at, via a real `Situation.SituationClient` gRPC call (default: the Host). Repointing that one setting drives an adapter against any other TacticalAPI implementation instead, independently of the others.
- **Config has no shared `Sources` nesting.** Each `Adapter.*`'s own `appsettings.json` has the source's settings directly under `Simulator:<SourceName>` (e.g. `Simulator:ConvoyEscort`), not nested under a `Sources` key — that nesting existed only when the Host held every source's config in one shared file, before the Host/adapter split, and has since been removed. When adding or fixing a source's `Options` class, bind to `SimulatorOptions.SectionName + ":<Name>"`, not anything Sources-shaped. The Host's own `appsettings.json` has no source config at all.
